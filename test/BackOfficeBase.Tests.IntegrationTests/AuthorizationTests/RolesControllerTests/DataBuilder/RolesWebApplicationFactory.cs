...
{
    public class RolesWebApplicationFactory<TStartup>
        ...
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            builder.ConfigureServices(services =>
            {
                ...
                services.AddDbContext<BackOfficeBaseDbContext>(options =>
                    options.UseNpgsql(string.Format(configuration.GetConnectionString(AppConfig.DefaultTestConnection), "Roles"))
                ...
                var logger = scopedServices.GetRequiredService<ILogger<RolesWebApplicationFactory<TStartup>>>();
                ...
                try
                {
                    ...
                    new TestDataBuilderForRoles(db).SeedData();
                    db.SaveChanges();
                }
                catch (Exception ex)
                {
